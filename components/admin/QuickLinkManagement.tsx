...
const LinkForm: React.FC<{ link?: QuickLink; onSave: (link: Omit<QuickLink, 'id'>, id?: string) => void; onCancel: () => void }> = ({ link, onSave, onCancel }) => {
    const [formData, setFormData] = useState<Omit<QuickLink, 'id'>>(link ? { ...link } : {
        title: '', category: '', url: '', description: '', status: 'active'
    });
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-700">All Quick Links</h2>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingLink ? 'Edit Quick Link' : 'Add New Quick Link'}>
                <LinkForm link={editingLink} onSave={handleSave} onCancel={() => setIsModalOpen(false)} />
            </Modal>
        </div>
    );
};
