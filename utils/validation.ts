...
export const validateJob = (job: Partial<Job>): string | null => {
    const requiredFields: Array<keyof Job> = ['title', 'department', 'category', 'description', 'qualification', 'vacancies', 'postedDate', 'lastDate', 'applyLink', 'status'];
    ...
    // Validate status
    if (job.status && !['active', 'closing-soon', 'expired'].includes(job.status)) {
