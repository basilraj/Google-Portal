...
const PreparationPage: React.FC<{ navigate: (path: string) => void }> = ({ navigate }) => {
    ...
    const [selectedCoursePlatform, setSelectedCoursePlatform] = useState('All Platforms'); // FIX: New state for course filter
    const [selectedBookCategory, setSelectedBookCategory] = useState('All Categories'); // FIX: New state for book filter
    ...
    // FIX: Memoized and filtered courses
    const filteredCourses = useMemo(() => {
        if (selectedCoursePlatform === 'All Platforms') {
            return preparationCourses;
        }
        return preparationCourses.filter(course => course.platform === selectedCoursePlatform);
    }, [preparationCourses, selectedCoursePlatform]);

    // FIX: Memoized and filtered books
    const filteredBooks = useMemo(() => {
        if (selectedBookCategory === 'All Categories') {
            return preparationBooks;
        }
        return preparationBooks.filter(book => book.category === selectedBookCategory);
    }, [preparationBooks, selectedBookCategory]);

    // FIX: Derive unique platforms for courses
    const uniqueCoursePlatforms = useMemo(() => {
        const platforms = preparationCourses.map(c => c.platform);
        return ['All Platforms', ...Array.from(new Set(platforms)).sort()];
    }, [preparationCourses]);

    // FIX: Derive unique categories for books
    const uniqueBookCategories = useMemo(() => {
        const categories = preparationBooks.map(b => b.category);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        {filteredCourses.length > 0 && (
                            <section>
                                <div className="flex justify-between items-center mb-6 pb-2 border-b-4 border-[var(--accent-color)]">
                                    <h2 className="text-3xl font-bold text-[#1e3c72]">Best Courses</h2>
                                    {/* FIX: Course Platform Filter */}
                                    {uniqueCoursePlatforms.length > 1 && (
                                        <select
                                            value={selectedCoursePlatform}
                                            onChange={(e) => setSelectedCoursePlatform(e.target.value)}
                                            className="px-3 py-2 border border-gray-300 rounded-md bg-white text-sm"
                                        >
                                            {uniqueCoursePlatforms.map(platform => (
                        ...
                        {filteredBooks.length > 0 && (
                             <section>
                                <div className="flex justify-between items-center mb-6 pb-2 border-b-4 border-[var(--accent-color)]">
                                    <h2 className="text-3xl font-bold text-[#1e3c72]">Top Recommended Books</h2>
                                    {/* FIX: Book Category Filter */}
                                    {uniqueBookCategories.length > 1 && (
                                        <select
                                            value={selectedBookCategory}
                                            onChange={(e) => setSelectedBookCategory(e.target.value)}
                                            className="px-3 py-2 border border-gray-300 rounded-md bg-white text-sm"
                                        >
                                            {uniqueBookCategories.map(category => (
