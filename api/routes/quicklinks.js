...
router.post('/', async (req, res, next) => {
    try {
        ...
        const id = uuidv4();
        const result = await req.db.query(`INSERT INTO QuickLink (id, title, category, url, description, status) VALUES ($1, $2, $3, $4, $5, $6)`, [id, linkData.title, linkData.category, linkData.url, linkData.description, linkData.status]);
        if (result.rowCount === 0) throw new Error('Failed to create quick link.');
