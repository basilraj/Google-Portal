...
router.get('/status', async (req, res, next) => {
    try {
        if (req.session.isAdmin && req.session.userId) {
            const userResult = await req.db.query('SELECT id, username, email FROM "User" WHERE id = $1', [req.session.userId]);
        ...
        const adminCountResult = await req.db.query('SELECT COUNT(*) AS count FROM "User"');
    ...
    try {
        ...
        switch (action) {
            ...
            case 'login': {
                ...
                const userResult = await req.db.query('SELECT * FROM "User" WHERE username = $1', [username]);
                const user = userResult.rows[0];
            ...
            case 'request_password_reset': {
                const userResult = await req.db.query('SELECT id FROM "User" WHERE email = $1', [email]);
                if (userResult.rows.length > 0) {
                    req.session.resetUserId = userResult.rows[0].id;
                    await req.session.save();
    ...
    try {
        ...
        switch (action) {
            case 'update_credentials': {
                ...
                const userResult = await req.db.query('SELECT * FROM "User" WHERE id = $1', [req.session.userId]);
                const user = userResult.rows[0];
                ...
                const updatedUserResult = await req.db.query('SELECT id, username, email FROM "User" WHERE id = $1', [req.session.userId]);
