...
router.put('/sponsored-ads', async (req, res, next) => {
    try {
        if (req.body.trackClick) {
            ...
            const updatedAdResult = await req.db.query(`SELECT * FROM SponsoredAd WHERE id = $1`, [adId]);
            await req.db.query('INSERT INTO ActivityLog (action, details) VALUES ($1, $2)', ['Sponsored Ad Updated', `Ad updated for ${updatedAdResult.rows[0].destinationUrl}`]);
            return res.status(200).json(updatedAdResult.rows[0]);
        }
    } catch (error) {
        next(error);
    }
...
router.post('/sponsored-ads', requireAdmin, async (req, res, next) => {
    try {
        ...
        const id = uuidv4();
        const result = await req.db.query(`INSERT INTO SponsoredAd (id, imageUrl, destinationUrl, placement, status, clicks) VALUES ($1, $2, $3, $4, $5, $6)`,
            [id, newItemData.imageUrl, newItemData.destinationUrl, newItemData.placement, newItemData.status, 0]
        );
        if (result.rowCount === 0) throw new Error('Failed to create sponsored ad.');
