...
router.post('/subscribers', async (req, res, next) => {
    try {
        ...
        const existingResult = await req.db.query('SELECT id FROM Subscriber WHERE email = $1', [email]);
        if (existingResult.rows.length > 0) {
            return res.status(409).json({ message: 'This email is already subscribed.' });
        }
        const id = uuidv4();
        const result = await req.db.query(`INSERT INTO Subscriber (id, email, status, "subscriptionDate") VALUES ($1, $2, $3, $4)`,
            [id, email, 'active', new Date()]
        );
        if (result.rowCount === 0) throw new Error('Failed to add subscriber.');
