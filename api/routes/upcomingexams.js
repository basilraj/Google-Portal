...
router.post('/', async (req, res, next) => {
    try {
        ...
        const id = uuidv4();
        const result = await req.db.query(`INSERT INTO UpcomingExam (id, name, deadline, notificationLink) VALUES ($1, $2, $3, $4)`, [id, newItemData.name, new Date(newItemData.deadline), newItemData.notificationLink]);
        if (result.rowCount === 0) throw new Error('Failed to create upcoming exam.');
